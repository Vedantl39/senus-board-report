...
- `artifacts/board-report/src/components/RiskRegister.jsx` — groups risks by category, ordered by `materiality_rank`, with `StatusPill` badges (red/amber/gray for New/Updated/Unchanged). Shows only the top 2 (most-material) risks per category by default with a "Show all in [category] (N more)" toggle (chevron + text, `lucide-react` `ChevronDown`), since real extraction brought the register to ~35 flat items.
- `artifacts/board-report/src/components/EventList.jsx` — shows the 5 most recent events by default (in whatever order the API already returns, i.e. `extracted_at DESC` — events have no structured date field, see Gotchas) with a "Show full history (N more)" toggle.
- `artifacts/board-report/src/components/MetricsTable.jsx` — each category is now a collapsible accordion section (only one open at a time, first category open by default) instead of always-expanded stacked tables, since detailed metrics (Growth & Revenue, Profitability, Cash & Liquidity, Balance Sheet) duplicate the hero KPI headline numbers.
- `artifacts/board-report/src/lib/metricsHelpers.js` — `findLatestMetric(metrics, name)` finds the most recent record for a specific metric name independent of the report's overall `latestPeriod()`. Needed for hero cards like Investors' Admission Price, which is a point-in-time fact only ever disclosed once (FY2025) and never re-disclosed in the latest period (H1 FY2026) — using the global latest period for that lookup falsely showed "Not available" despite the data existing.
...
- Extracted governance events have no structured date field (Claude only extracts `title`/`description`, and many events' `period_label` is `null`), so `EventList`'s "most recent" default ordering relies on the API's existing `extracted_at DESC` sort rather than a true event date — acceptable given the extraction taxonomy, but not a real chronological guarantee if extraction order ever changes.
