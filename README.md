...
2. **Automated tests** — 98 tests across the derived metrics calculator, the commentary grounding validator, the extraction pipeline, the document upload endpoint, and the API layer (real HTTP integration tests against a running server, not mocked).
...
npm install
npm test              # run the backend test suite (98 tests)
