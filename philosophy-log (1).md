...
## A Metric Defaulting Missing Inputs to Zero Instead of Null — Found and Regression-Tested
**Decision:** `totalAssets()` was defaulting all five missing balance-sheet inputs to 0 when none were disclosed in a document, producing a fabricated "€0 total assets" row instead of correctly returning "not available." Found during a full manual audit of the deployed app, fixed at the source, and now covered by an explicit regression test (`assert.equal(totalAssets(noBalanceSheetData), null)`).
**Philosophy:** This is the same class of bug as the earlier `safeDivide` undefined-vs-null issue from Bucket 5 — a derived metric silently converting "we don't know" into a specific, confident-looking wrong number. A document with no balance sheet data at all (a press release, not a financial statement) should never produce a total assets figure; that it did was found by checking the deployed app's actual output against what should be possible given the source document, not by trusting that "the tests pass" meant the pipeline was correct in every case.
**Where it shows up:** `deriveMetrics.js`, `deriveMetrics.test.js`.
**Proves:** Software Engineering, Technical Decision-Making

## A Sign-Flip Bug That "Passed" Once Reappeared — the Real Cause Was in the Comparative Period, Not the Display Logic
**Decision:** The Net Assets/(Liabilities) sign-flip display (side-by-side values instead of a misleading percentage across a sign change) was confirmed working, then later appeared broken again on the Lenders view. Investigation found the display logic itself was untouched — the actual cause was that the specific comparative period being shown for H1 FY2026 (the source document's own "31 December 2024" figure, also positive) masked the real sign flip, which only appears against the true prior full financial year (FY2025, negative).
**Philosophy:** A bug that looks like a regression isn't always one — diffing the actual commits to rule out an accidental break, rather than assuming recent changes were the cause, found the real explanation: which comparative period a metric is measured against can itself determine whether a "sign flip" is even detected. Every other metric was checked for the same latent issue as part of the fix, not just the one instance that was reported.
**Where it shows up:** The Lenders view's Solvency & Leverage hero card.
**Proves:** Software Engineering, Technical Decision-Making

## Governance Events Missing From Two of Three Views That Should Have Had Them
**Decision:** A full audit found Governance Events were missing from both the Investors and Management views, despite both being specified to include events per the original view definitions — only Lenders was designed to exclude them. Both fixed by adding the missing events fetch to each view's endpoint.
**Philosophy:** This is exactly why a full page-by-page audit against the documented design matters even after individual features seem to work in isolation — the Investors and Management omissions weren't caught by testing each feature separately, only by systematically checking every view against what it was actually supposed to show.
**Where it shows up:** `/api/views/investors` and `/api/views/management` endpoints.
