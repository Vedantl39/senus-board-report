...
**Demo access:** this app uses a single shared password for the login screen — `Vedant@123`. This is a deliberate, documented choice for a non-multi-tenant assessment context (see the Philosophy Log's "Single Login, View Switcher" entry), confirmed acceptable by Assiduous directly. The password is also shown on the login screen itself.
...
- **Measure / Report / Verify navigation** — Senus's own stated MRV framing, applied structurally: raw disclosed data (Measure), the polished audience-specific dashboard (Report), and AI-generated commentary checked against the source data before display (Verify)
- **Upload tab** — new source documents (PDF or spreadsheet) can be uploaded directly and run through the same real extraction pipeline that built the rest of the report; new disclosures appear across every view as soon as extraction finishes
- **Ask a follow-up question** (Verify tab) — a grounded Q&A box using the same commentary-generation and validation pipeline, so a board member can interrogate the data directly rather than only reading a static paragraph
- **PDF export** — any audience view can be exported as a branded PDF, retaining source traceability and audited/unaudited badges
- **Live share price indicator** — ticker and latest price shown in the header, sourced from the ingested share price data, clearly timestamped as a snapshot rather than a live feed (per the client's confirmation that a static snapshot is acceptable)
...
- Two genuinely comparable financial periods exist: FY2025 (audited, consolidated — pre-Loamin) and H1 FY2026 (unaudited, consolidated — post-Loamin) — the consolidation scope difference (Loamin acquisition, Nov 2025) is tracked, not blended away. An earlier pass through this project mislabeled FY2025 as "standalone"; re-running real AI extraction against the primary source document caught and corrected this — see the Philosophy Log.
...
2. **Automated tests** — 100 tests across the derived metrics calculator, the commentary grounding validator, the extraction pipeline, the document upload endpoint, and the API layer (real HTTP integration tests against a running server, not mocked).
...
5. **A full manual page-by-page audit of the deployed app**, done after everything appeared to be working, checking every figure on every one of the four audience views against the real source documents and the brief's named requirements. This caught several real bugs that automated tests alone hadn't: duplicate blank-period rows from a secondary source document, an EBITDA-to-FCF bridge chart that gave a different (and incorrect) answer on one audience view than another for the identical period, a metric silently defaulting missing balance-sheet inputs to zero instead of "not available," and a misleading percentage shown across a genuine sign flip in Net Assets/Liabilities. Each was root-caused, not just patched — the zero-defaulting bug now has its own regression test.
...
artifacts/
  api-server/                       — backend (plain JS, Express)
    ...
    test/                           — 100 tests, run with `npm test`
...
npm install
npm test              # run the backend test suite (100 tests)
node src/index.js     # start the API server (requires DATABASE_URL, ANTHROPIC_API_KEY)

cd artifacts/board-report
npm install
npm test               # run the frontend test suite
npm run dev             # start the dev frontend
```

## Known limitations

- **CI/CD is not yet live.** A GitHub Actions workflow (`.github/workflows/test.yml`) exists and is committed, but the connected GitHub integration's OAuth token lacks the `workflow` scope needed to push it — so it isn't currently running on GitHub. Stated here explicitly rather than left for someone else to discover; not a priority fix given the assessment's timeframe.
- **Embeddings/semantic search** ("find similar risks") was considered, per a related job description's emphasis on this skill, and deliberately deferred — if built, the plan is for Claude to judge similarity directly via a prompt (consistent with how risk status diffing already works in this project), rather than introducing a new embeddings provider for a secondary feature.
